...
#define ICON_BOX_Y_DIFFERENCE       24
static void CreatePartyMonIcons()
{
    ...
    for(i = 0; i < gPlayerPartyCount; i++)
    {
        ...
        sShopData->iconMonSpriteIds[i] = CreateMonIcon(GetMonData(&gPlayerParty[i], MON_DATA_SPECIES_OR_EGG), SpriteCB_MonIcon, x, y - 2, 0, GetMonData(&gPlayerParty[i], MON_DATA_PERSONALITY));

        gSprites[sShopData->iconMonSpriteIds[i]].oam.priority = 0;
    }
}
...
static void TintPartyMonIcons(u16 move)
{
    ...
    for (i = 0; i < gPlayerPartyCount; i++)
    {
        ...
        {
            gSprites[sShopData->iconMonSpriteIds[i]].oam.objMode = ST_OAM_OBJ_BLEND;
        }
        else
        {
            gSprites[sShopData->iconMonSpriteIds[i]].oam.objMode = ST_OAM_OBJ_NORMAL;
        }
    }
...
static void SortItemsByName(u16 *buffer, u16 count)
{
    ...
    {
        items[i].id = buffer[i];
        items[i].name = GetItemName(buffer[i]);
    }
...
static void CleanUpShopItemsForSale()
{
      ...
      || FlagGet(FLAG_SORT_SHOP_ITEMS))
    {
        ...
        while (sMartInfo.itemSource[i] && uniqueCount < sMartInfo.itemCount)
        {
            ...
            const u8 *currentName = GetItemName(current);
            ...
            {
                if (!StringCompare(GetItemName(sUniqueItemBuffer[j]), currentName))
                {
                    isDuplicate = TRUE;
                    break;
                }
...
static void CB2_InitBuyMenu(void)
{
    ...
    case 0:
        ...
        sShopData->iconMonSpriteIds[0] = SPRITE_NONE;
        sShopData->iconMonSpriteIds[1] = SPRITE_NONE;
        sShopData->iconMonSpriteIds[2] = SPRITE_NONE;
...
static void BuyMenuPrintItemDescriptionAndShowItemIcon(s32 item, bool8 onInit, struct ListMenu *list)
{
    ...
    BuyMenuPrintSmall(WIN_ITEM_DESCRIPTION, description, 3, 1 + 16, 0, COLORID_NORMAL);
    ...
    if (item != LIST_CANCEL)
    {
        if (sMartInfo.martType == MART_TYPE_NORMAL)
        {
            if (IsTM(item))
            {
                ...
                {
                    move = gItemsInfo[item].secondaryId;
                }
...
static void BuyMenuPrintPriceInList(u8 windowId, u32 itemId, u8 y)
{
    ...
    if (itemId != LIST_CANCEL)
    {
        ...
        {
            StringExpandPlaceholders(gStringVar4, gText_BPVar1);
        }
...
static void BuyMenuPrintSmall(u8 windowId, const u8 *text, u8 x, u8 y, s8 speed, u8 colorSet)
{
    AddTextPrinterParameterized4(windowId, FONT_SMALL_NARROW, x, y, 0, 4, sShopBuyMenuTextColors[colorSet], speed, text);
}
...
static void BuyMenuDrawGraphics(void)
{
    ...
    if (sMartInfo.martType == MART_TYPE_NORMAL)
        ...
        PrintBPMoneyAmountInMoneyBoxWithBorder(WIN_MONEY, 1, 13, gSaveBlock2Ptr->secretBaseShopCoins);
...
static void Task_BuyMenu(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        default:
            ...
            if (GetItemImportance(itemId) && (CheckBagHasItem(itemId, 1) || CheckPCHasItem(itemId, 1)) && (sMartInfo.martType == MART_TYPE_NORMAL))
            {
                BuyMenuDisplayMessage(taskId, gText_ThatItemIsSoldOut, BuyMenuReturnToItemList);
            }
            else if ((GetItemPrice(itemId) == 0) && (sMartInfo.martType == MART_TYPE_NORMAL))
            {
                BuyMenuDisplayMessage(taskId, gText_ThatItemIsSoldOut, BuyMenuReturnToItemList);
            }
...
static void Task_BuyHowManyDialogueInit(u8 taskId)
{
    ...
    if (sMartInfo.martType == MART_TYPE_NORMAL)
    {
        ...
        maxQuantity = gSaveBlock2Ptr->secretBaseShopCoins / sShopData->totalCost;
...
static void Task_BuyHowManyDialogueHandleInput(u8 taskId)
{
    ...
    else
    {
        ...
        else if (JOY_NEW(B_BUTTON))
        {
            ...
            if(FlagGet(FLAG_TM_SHOP))
            {
                gSprites[sShopData->itemSpriteIds[0]].invisible = FALSE;
                gSprites[sShopData->itemSpriteIds[1]].invisible = FALSE;
            }
...
static void BuyMenuSubtractCoins(u8 taskId)
{
    ...
    if (gSaveBlock2Ptr->secretBaseShopCoins < sShopData->totalCost)
        gSaveBlock2Ptr->secretBaseShopCoins = 0;
    else
        gSaveBlock2Ptr->secretBaseShopCoins -= sShopData->totalCost;

    PlaySE(SE_SHOP);
    PrintBPMoneyAmountInMoneyBox(WIN_MONEY, gSaveBlock2Ptr->secretBaseShopCoins, 0);
...
static void BuyMenuTryMakePurchase(u8 taskId)
{
    ...
    else
    {
        ...
        {
            if (sMartInfo.martType == MART_TYPE_DECOR)
                BuyMenuDisplayMessage(taskId, gText_ThankYouIllSendItHome, BuyMenuSubtractCoins);
            else // MART_TYPE_DECOR2
                BuyMenuDisplayMessage(taskId, gText_ThanksIllSendItHome, BuyMenuSubtractCoins);
        }
        else
        {
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId)
{
    ...
    if (sMartInfo.martType == MART_TYPE_NORMAL)
        ...
        PrintBPMoneyAmount(WIN_QUANTITY_PRICE, CalculateMoneyTextHorizontalPosition(sShopData->totalCost), 1, sShopData->totalCost, TEXT_SKIP_DRAW);
