...
{
    const u16 *effect = GetItemEffect(item);
...
void Task_AbilityCapsule(u8 taskId)
{
    ...
    case 0:
        // Can't use.
        if (GetSpeciesAbility(seededSpecies, 0) == GetSpeciesAbility(seededSpecies, 1)
            || GetSpeciesAbility(seededSpecies, 1) == 0
            || tAbilityNum > 1
            || !seededSpecies)
        {
            gPartyMenuUseExitCallback = FALSE;
...
void Task_AbilityPatch(u8 taskId)
{
    ...
    case 0:
        // Can't use.
        if (GetSpeciesAbility(seededSpecies, tAbilityNum) == 0
            || !seededSpecies
            )
        {
            gPartyMenuUseExitCallback = FALSE;
...
{
    const u16 *effect = GetItemEffect(gSpecialVar_ItemId);
...
{
    u32 statusCure;
    const u16 *itemEffect = GetItemEffect(item);
