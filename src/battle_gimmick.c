...
bool32 IsGimmickSelected(u32 battler, enum Gimmick gimmick)
{
    ...
    if (TESTING)
        return (gBattleStruct->gimmick.toActivate & (1u << battler)) && gBattleStruct->gimmick.chosenGimmick[battler] == gimmick;
    else
...
{
    if (IsDoubleBattle() && IsPartnerMonFromSameTrainer(battler) && gBattleStruct->gimmick.chosenGimmick[BATTLE_PARTNER(battler)] == gimmick)
    {
