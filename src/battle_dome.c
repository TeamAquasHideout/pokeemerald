...
static void ResolveDomeRoundWinners(void)
{
    ...
    if (gSpecialVar_0x8005 == DOME_PLAYER_WON_MATCH)
    {
        ...
        DOME_TRAINERS[TrainerIdToTournamentId(TRAINER_BATTLE_PARAM.opponentA)].eliminatedAt = gSaveBlock1Ptr->frontier.curChallengeBattleNum;
        gSaveBlock1Ptr->frontier.domeWinningMoves[TrainerIdToTournamentId(TRAINER_BATTLE_PARAM.opponentA)] = gBattleResults.lastUsedMovePlayer;
...
static u16 GetWinningMove(int winnerTournamentId, int loserTournamentId, u8 roundId)
{
    ...
    do
    {
        ...
        {
            if (gSaveBlock1Ptr->frontier.domeWinningMoves[GetOpposingNPCTournamentIdByRound(winnerTournamentId, i)] == moves[j])
                break;
        }
        if (i != roundId - 1)
        {
