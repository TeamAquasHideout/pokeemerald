...
static const u16 sPrizeListRoar[]        = {ITEM_TM_BRICK_BREAK,          ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
...
static const u16 sPrizeListAttract[]     = {ITEM_TM_GIGA_DRAIN,       ITEM_ETHER, ITEM_MAX_POTION, ITEM_REVIVE, ITEM_FLUFFY_TAIL, ITEM_GREAT_BALL};
...
void SetHillTrainerFlag(void)
{
    ...
    for (i = 0; i < HILL_TRAINERS_PER_FLOOR; i++)
    {
        if (gSaveBlock1Ptr->frontier.trainerIds[i] == TRAINER_BATTLE_PARAM.opponentA)
        {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS)
    {
        for (i = 0; i < HILL_TRAINERS_PER_FLOOR; i++)
        {
            if (gSaveBlock1Ptr->frontier.trainerIds[i] == TRAINER_BATTLE_PARAM.opponentB)
            {
                gSaveBlock1Ptr->frontier.trainerFlags |= 1u << (trainerIndexStart + i);
                break;
            }
