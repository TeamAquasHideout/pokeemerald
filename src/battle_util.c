...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        {
            u32 abilityAtk = GetBattlerAbility(gBattlerAttacker);
            if ((B_POWDER_GRASS < GEN_6 || !IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_GRASS))
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case ABILITY_NORMALIZE:
        if (moveType == TYPE_NORMAL && gBattleStruct->battlerState[battlerAtk].ateBoost && GetGenConfig(GEN_CONFIG_ATE_MULTIPLIER) >= GEN_7)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    case ABILITY_ARCANIST:
        if (moveType == TYPE_PSYCHIC)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    case ABILITY_EERIE_AURA:
    ...
    case ABILITY_FAFNIR:
        if (moveType == TYPE_ELECTRIC || moveType == TYPE_DRAGON)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    case ABILITY_OMNIHUNTER:
        if (moveType == TYPE_FLYING)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_SINGULARITY:
        if (moveType == TYPE_FIRE || moveType == TYPE_ICE)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.25));
        break;
    case ABILITY_VENOM_BLOOD:
...
u32 TryImmunityAbilityHealStatus(u32 battler, u32 caseID)
{
    ...
    case ABILITY_OBLIVIOUS:
        if (gBattleMons[battler].volatiles.infatuation)
            effect = 3;
        else if (B_OBLIVIOUS_TAUNT >= GEN_6 && gDisableStructs[battler].tauntTimer != 0)
            effect = 4;
        break;
    }
...
u32 GetCalcedMoveBasePower(u32 move, u32 battlerAtk, u32 weather)
{
    ...
    case EFFECT_STORED_POWER:
        ...
        break;
    case EFFECT_ECHOED_VOICE:
        // gBattleStruct->metronomeItemCounter incremented in ppreduce
        if (gBattleStruct->metronomeItemCounter[battlerAtk] != 0)
        {
            basePower += (basePower * gBattleStruct->metronomeItemCounter[battlerAtk]);
            if (basePower > 200)
                basePower = 200;
        }
        break;
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    case ABILITY_VICTORY_STAR:
        ...
        break;
    case ABILITY_ABYSS_GRACE:
        calc = (calc * 110) / 100; // The Pit: 1.1 team accuracy boost
        break;
    case ABILITY_HUSTLE:
        if (IsBattleMovePhysical(move))
    ...
    case ABILITY_ABYSS_GRACE:
        if (IsBattlerAlive(atkAlly))
            calc = (calc * 110) / 100; // The Pit: ally's 1.1 team accuracy boost
        break;
    }
