...
static const u8 *GetInteractedObjectEventScript(struct MapPosition *position, u8 metatileBehavior, u8 direction)
{
    ...
    // rave: ripping this out, reapply duct tape fix in case of emergency
    // if((VarGet(VAR_PIT_FLOOR) > 0) && (VarGet(VAR_PIT_FLOOR) % 5))
    // {
    //     if(((gSpecialVar_LastTalked > 0) && (gSpecialVar_LastTalked < 9)))
    //     {
