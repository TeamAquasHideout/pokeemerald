...
static void SetTowerBattleWon(void)
{
    ...
    if (TRAINER_BATTLE_PARAM.opponentA == TRAINER_EREADER)
        ClearEReaderTrainer(&gSaveBlock1Ptr->frontier.ereaderTrainer);
#endif //FREE_BATTLE_TOWER_E_READER
...
static void SetNextFacilityOpponent(void)
{
    ...
    else
    {
        ...
        if (battleMode == FRONTIER_MODE_MULTIS || battleMode == FRONTIER_MODE_LINK_MULTIS)
        {
            ...
            TRAINER_BATTLE_PARAM.opponentA = gSaveBlock1Ptr->frontier.trainerIds[id * 2];
            TRAINER_BATTLE_PARAM.opponentB = gSaveBlock1Ptr->frontier.trainerIds[id * 2 + 1];
        ...
        {
            SetBattleFacilityTrainerGfxId(TRAINER_BATTLE_PARAM.opponentA, 0);
            gSaveBlock1Ptr->frontier.trainerIds[gSaveBlock1Ptr->frontier.curChallengeBattleNum] = TRAINER_BATTLE_PARAM.opponentA;
        }
        else
        {
            s32 i;
...
void SetBattleFacilityTrainerGfxId(u16 trainerId, u8 tempVarId)
{
    ...
    {
        facilityClass = gSaveBlock1Ptr->frontier.ereaderTrainer.facilityClass;
    }
...
u8 GetFrontierTrainerFrontSpriteId(u16 trainerId)
{
    ...
    if (trainerId == TRAINER_EREADER)
    {
        return gFacilityClassToPicIndex[gSaveBlock1Ptr->frontier.ereaderTrainer.facilityClass];
    }
    else if (trainerId == TRAINER_FRONTIER_BRAIN)
#else
    if (trainerId == TRAINER_FRONTIER_BRAIN)
...
enum TrainerClassID GetFrontierOpponentClass(u16 trainerId)
{
    ...
    if (trainerId == TRAINER_EREADER)
    {
        trainerClass = gFacilityClassToTrainerClass[gSaveBlock1Ptr->frontier.ereaderTrainer.facilityClass];
    }
    else if (trainerId == TRAINER_FRONTIER_BRAIN)
#else
    if (trainerId == TRAINER_FRONTIER_BRAIN)
...
void GetFrontierTrainerName(u8 *dst, u16 trainerId)
{
    ...
    if (trainerId == TRAINER_EREADER)
    {
        ...
        for (i = 0; i < PLAYER_NAME_LENGTH; i++)
            dst[i] = gSaveBlock1Ptr->frontier.ereaderTrainer.name[i];
    #endif //FREE_BATTLE_TOWER_E_READER
    }
    else if (trainerId == TRAINER_FRONTIER_BRAIN)
    {
...
static void FillTrainerParty(u16 trainerId, u8 firstMonId, u8 monCount)
{
    ...
    else if (trainerId == TRAINER_EREADER)
    {
        ...
        for (i = firstMonId; i < firstMonId + FRONTIER_PARTY_SIZE; i++)
            CreateBattleTowerMon(&gEnemyParty[i], &gSaveBlock1Ptr->frontier.ereaderTrainer.party[i - firstMonId]);
    #endif //FREE_BATTLE_TOWER_E_READER
        return;
    }
    else if (trainerId == TRAINER_FRONTIER_BRAIN)
...
#if FREE_BATTLE_TOWER_E_READER == FALSE
    if (trainerId == TRAINER_EREADER)
        FrontierSpeechToString(gSaveBlock1Ptr->frontier.ereaderTrainer.greeting);
    else if (trainerId < FRONTIER_TRAINERS_COUNT)
#else
    if (trainerId < FRONTIER_TRAINERS_COUNT)
#endif //FREE_BATTLE_TOWER_E_READER
...
void DoSpecialTrainerBattle(void)
{
    ...
    #if FREE_BATTLE_TOWER_E_READER == FALSE
        ZeroEnemyPartyMons();
        for (i = 0; i < (int)ARRAY_COUNT(gSaveBlock1Ptr->frontier.ereaderTrainer.party); i++)
            CreateBattleTowerMon(&gEnemyParty[i], &gSaveBlock1Ptr->frontier.ereaderTrainer.party[i]);
...
static void LoadMultiPartnerCandidatesData(void)
{
    ...
    for (i = 0; i < BATTLE_TOWER_RECORD_COUNT; i++)
    {
        ...
        if (gSaveBlock1Ptr->frontier.towerRecords[i].winStreak / FRONTIER_STAGES_PER_CHALLENGE <= challengeNum
            && gSaveBlock1Ptr->frontier.towerRecords[i].lvlMode == lvlMode
            && recordHasData
            && gSaveBlock1Ptr->frontier.towerRecords[i].checksum == checksum)
        {
...
static void LoadLinkMultiOpponentsData(void)
{
    ...
    case 3:
        if ((GetBlockReceivedStatus() & 3) == 3)
        {
            ...
            TRAINER_BATTLE_PARAM.opponentA = gSaveBlock1Ptr->frontier.trainerIds[battleNum * 2];
            TRAINER_BATTLE_PARAM.opponentB = gSaveBlock1Ptr->frontier.trainerIds[battleNum * 2 + 1];
            SetBattleFacilityTrainerGfxId(TRAINER_BATTLE_PARAM.opponentA, 0);
            SetBattleFacilityTrainerGfxId(TRAINER_BATTLE_PARAM.opponentB, 1);
...
static void SetTowerInterviewData(void)
{
    ...
    GetBattleTowerTrainerLanguage(&gSaveBlock1Ptr->frontier.towerInterview.opponentLanguage, TRAINER_BATTLE_PARAM.opponentA);
    gSaveBlock1Ptr->frontier.towerInterview.opponentSpecies = GetMonData(GetBattlerMon(1), MON_DATA_SPECIES, NULL);
    gSaveBlock1Ptr->frontier.towerInterview.playerSpecies = GetMonData(GetBattlerMon(0), MON_DATA_SPECIES, NULL);
...
{
#if FREE_BATTLE_TOWER_E_READER == FALSE
    struct BattleTowerEReaderTrainer *ereaderTrainer = &gSaveBlock1Ptr->frontier.ereaderTrainer;
    s32 i, j;
...
{
#if FREE_BATTLE_TOWER_E_READER == FALSE
    return gFacilityClassToTrainerClass[gSaveBlock1Ptr->frontier.ereaderTrainer.facilityClass];
#else
    return 0;
#endif //FREE_BATTLE_TOWER_E_READER
}
...
void ValidateEReaderTrainer(void)
{
    ...
    gSpecialVar_Result = FALSE;
    ereaderTrainer = &gSaveBlock1Ptr->frontier.ereaderTrainer;
    ...
    if (gSaveBlock1Ptr->frontier.ereaderTrainer.checksum != checksum)
    {
        ClearEReaderTrainer(&gSaveBlock1Ptr->frontier.ereaderTrainer);
        gSpecialVar_Result = TRUE;
    }
...
static void CopyEReaderTrainerFarewellMessage(void)
{
    ...
    if (gBattleOutcome == B_OUTCOME_DREW)
        ...
        FrontierSpeechToString(gSaveBlock1Ptr->frontier.ereaderTrainer.farewellPlayerWon);
    else
        FrontierSpeechToString(gSaveBlock1Ptr->frontier.ereaderTrainer.farewellPlayerLost);
#endif //FREE_BATTLE_TOWER_E_READER
}
