...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags, bool8 isPlayer)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        if(!isPlayer)
        {
            if ((VarGet(VAR_PIT_FLOOR) <= 25) || (gSaveBlock2Ptr->mode3MonsOnly == MON_AMOUNT_3))
                monsCount = 3;
        ...
        {   
            ...
            else
            {
                ...
                else
                {
                    ...
                    //     CreateMon(&party[i], SPECIES_PALAFIN_ZERO, monLevel, 0, TRUE, personalityValue, otIdType, fixedOtId);
                    // else if (i == 1)
