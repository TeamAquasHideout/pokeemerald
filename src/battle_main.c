...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    //more
    [TRAINER_CLASS_PIT_BOSS] = { _("PIT BOSS") },
};
...
u8 CreateNPCTrainerPartyFromTrainer(struct Pokemon *party, const struct Trainer *trainer, bool32 firstTrainer, u32 battleTypeFlags, bool8 isPlayer)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        if(!isPlayer)
        {
            if ((VarGet(VAR_PIT_FLOOR) <= 25) || (gSaveBlock2Ptr->mode3MonsOnly == PARTY_SIZE_3))
                monsCount = 3;
        ...
        {   
            ...
            else
            {
                ...
                else if (monLevel == 100 || monLevel % 25 == 0)
                {
                    ...
                    //     CreateMon(&party[i], SPECIES_PERSIAN, monLevel, 0, TRUE, personalityValue, otIdType, fixedOtId);
                    // else if (i == 1)
                    //     CreateMon(&party[i], SPECIES_BLISSEY, monLevel, 0, TRUE, personalityValue, otIdType, fixedOtId);
                    // else if (i == 2)
                    //     CreateMon(&party[i], SPECIES_MEOWTH, monLevel, 0, TRUE, personalityValue, otIdType, fixedOtId);
