...
void TryPutFrontierTVShowOnAir(u16 winStreak, u8 facilityAndMode)
{
    ...
    if (sCurTVShowSlot != -1)
    {
        ...
        case FRONTIER_SHOW_TOWER_LINK_MULTIS:
            show->frontier.species1 = GetMonData(GetSavedPlayerPartyMon(gSaveBlock1Ptr->frontier.selectedPartyMons[0] - 1), MON_DATA_SPECIES, NULL);
            show->frontier.species2 = GetMonData(GetSavedPlayerPartyMon(gSaveBlock1Ptr->frontier.selectedPartyMons[1] - 1), MON_DATA_SPECIES, NULL);
            break;
        }
        StorePlayerIdInRecordMixShow(show);
