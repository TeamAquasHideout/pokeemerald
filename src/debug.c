...
// Script menu
static const u8 sDebugText_Util_Script_1[] = _("Clear Current Floor");
static const u8 sDebugText_Util_Script_2[] = _("Floor 25 (No Candy)");
static const u8 sDebugText_Util_Script_3[] = _("Advance To Archie");
static const u8 sDebugText_Util_Script_4[] = _("Show HoF screen");
...
static void DebugAction_Sound_MUS_SelectId(u8 taskId)
{
    if (JOY_NEW(DPAD_ANY))
    {
        ...
        {
            gTasks[taskId].tInput += sPowersOfTen[gTasks[taskId].tDigit];
            if (gTasks[taskId].tInput > SONGS_END)
                gTasks[taskId].tInput = SONGS_END;
        }
        if (JOY_NEW(DPAD_DOWN))
        {
            gTasks[taskId].tInput -= sPowersOfTen[gTasks[taskId].tDigit];
            if (gTasks[taskId].tInput < SONGS_START)
                gTasks[taskId].tInput = SONGS_START;
        }
        if (JOY_NEW(DPAD_LEFT))
        {
