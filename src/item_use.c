...
static const u8 sText_CantUseEscapeRope[] = _("It failed!\pIt seems there's no escape from\nThe Pit...{PAUSE_UNTIL_PRESS}");
...
void DisplayDadsAdviceCannotUseItemMessage(u8 taskId, bool8 isUsingRegisteredKeyItemOnField)
{
    if (gSpecialVar_ItemId == ITEM_ESCAPE_ROPE)
        DisplayCannotUseItemMessage(taskId, isUsingRegisteredKeyItemOnField, sText_CantUseEscapeRope);
    else
...
bool32 CannotUseItemsInBattle(u16 itemId, struct Pokemon *mon)
{
    ...
    case EFFECT_ITEM_CURE_STATUS:
        ...
        if (!((GetMonData(mon, MON_DATA_STATUS) & GetItemStatus1Mask(itemId))
          || (target != PARTY_SIZE && SelectedMonHasVolatile(itemId))))
            cannotUse = TRUE;
        break;
