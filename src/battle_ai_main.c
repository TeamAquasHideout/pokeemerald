...
void Ai_InitPartyStruct(void)
{
    ...
    {
        if (gAiPartyData->mons[B_SIDE_OPPONENT][i].gimmick == GIMMICK_PIT_NONE
          && !(((gSpecialVar_TrainerNumber == TRAINER_RANDOM_PIT_BOSS) || (gSpecialVar_TrainerNumber == TRAINER_RANDOM_PIT_BOSS_DOUBLES)) && i == (gAiPartyData->count[B_SIDE_OPPONENT] - 1))) //exclude Ace
        {
            //Dynamax
            u16 dynamax_odds = 0;
            ...
            {
                gAiPartyData->mons[B_SIDE_OPPONENT][i].gimmick = GIMMICK_PIT_DYNA;
                hasDyna = TRUE;
            }
            else if (FlagGet(FLAG_TERA_ACTIVE) && RandomPercentage(RNG_NONE, tera_odds) && !hasTera)
            {
                gAiPartyData->mons[B_SIDE_OPPONENT][i].gimmick = GIMMICK_PIT_TERA;
                hasTera = TRUE;
            }
        }
    }
